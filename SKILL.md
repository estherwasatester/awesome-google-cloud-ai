...
3. Read the JSON output and perform the following installations:
   - **MCP Servers**: Configure the MCP servers (if tools are available to configure MCPs, or output instructions).
   - **Gemini CLI Extensions**: Install the extensions (e.g., `gemini-cli extension install <url>`).
   - **Agent Skills**: Fetch and install the agent skills (e.g., clone the repo or use `gemini-cli skill add <url>`).
4. Once all tools are configured or the steps are generated, provide a comprehensive summary to the user outlining what was installed and that the workspace is ready for development.
